...
if (${env:APPVEYOR_JOB_NAME} -eq "Build Xirorig_Native Windows") {
    ...
    cmd /c "call 'C:\Program Files (x86)\Microsoft Visual Studio\2019\Community\VC\Auxiliary\Build\vcvarsamd64_arm.bat' && cmake -G 'Visual Studio 16 2019' -A ARM -DCMAKE_BUILD_TYPE=Release -DCMAKE_INSTALL_PREFIX=$PROJECT_ROOT/${env:XIRORIG_NATIVE_ROOT}/out/install/ARM32-windows -DVCPKG_TARGET_TRIPLET=arm-windows-static -DEXCLUDE_CPU_INFO_DEPENDENCY=true -S $PROJECT_ROOT/${env:XIRORIG_NATIVE_ROOT} -B $PROJECT_ROOT/${env:XIRORIG_NATIVE_ROOT}/out/build/ARM32-windows && msbuild.exe ${env:APPVEYOR_BUILD_FOLDER}\${env:XIRORIG_NATIVE_ROOT}\out\build\ARM32-windows\INSTALL.vcxproj -p:Configuration=Release"

    # Build Xirorig Native ARM64
    cmd /c "call 'C:\Program Files (x86)\Microsoft Visual Studio\2019\Community\VC\Auxiliary\Build\vcvarsamd64_arm64.bat' && cmake -G 'Visual Studio 16 2019' -A ARM64 -DCMAKE_BUILD_TYPE=Release -DCMAKE_INSTALL_PREFIX=$PROJECT_ROOT/${env:XIRORIG_NATIVE_ROOT}/out/install/ARM64-windows -DVCPKG_TARGET_TRIPLET=arm64-windows-static -DEXCLUDE_CPU_INFO_DEPENDENCY=true -S $PROJECT_ROOT/${env:XIRORIG_NATIVE_ROOT} -B $PROJECT_ROOT/${env:XIRORIG_NATIVE_ROOT}/out/build/ARM64-windows && msbuild.exe ${env:APPVEYOR_BUILD_FOLDER}\${env:XIRORIG_NATIVE_ROOT}\out\build\ARM64-windows\INSTALL.vcxproj -p:Configuration=Release"
