...
internal class PBKDF1 : IDisposable
{
    private static class Native
    {
        ...
        [DllImport(Program.XenoNativeLibrary)]
        public static extern int KeyDerivationFunctionUtility_GetBytes(IntPtr ctx, in byte rgbOut, int cb);
    ...
    {
        _context = Native.KeyDerivationFunctionUtility_CreatePBKDF1(MemoryMarshal.GetReference(password), password.Length, MemoryMarshal.GetReference(salt), salt.Length, iterations, hashName);
    }
    ...
    {
        return Native.KeyDerivationFunctionUtility_GetBytes(_context, MemoryMarshal.GetReference(rgbOut), rgbOut.Length);
    }

    public int GetBytes(Span<byte> rgbOut, int cb)
    {
        return Native.KeyDerivationFunctionUtility_GetBytes(_context, MemoryMarshal.GetReference(rgbOut), cb);
    }
