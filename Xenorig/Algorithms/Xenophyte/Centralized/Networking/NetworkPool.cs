...
public delegate void NewBlockEventHandler(in BlockHeaderStack blockHeader);

public delegate void ConnectionErrorHandler();

public sealed partial class NetworkPool : IDisposable
{
    ...
    public string UserAgent => _pool.UserAgent;
    ...
    private TcpClient? _getBlockHeaderTcpClient;
    private TcpClient? _sentBlockTcpClient;
    ...
    public NetworkPool(ILogger logger, XenorigOptions options, Pool pool)
    {
        ...
        _options = options;
        _pool = pool;
        _httpClient = new HttpClient { DefaultRequestHeaders = { UserAgent = { ProductInfoHeaderValue.Parse(_pool.UserAgent) } } };
    }

    private static ArrayOwner<byte> GenerateCertificate(out int length)
    {
    ...
    private async Task ConnectAsync(CancellationToken cancellationToken)
    {
        ...
        try
        {
            await _connectionSemaphoreSlim.WaitAsync(cancellationTokenSource.Token).ConfigureAwait(false);

            do
            {
                _getBlockHeaderTcpClient = new TcpClient();
                _getBlockHeaderTcpClient.NoDelay = true;
                ...
                using (var connectionTimeoutTokenSource = CancellationTokenSource.CreateLinkedTokenSource(cancellationTokenSource.Token, networkTimeoutCts.Token))
                {
                    try
                    {
                        ...
                        {
                            await _getBlockHeaderTcpClient.ConnectAsync(ipEndPoint.Address, ipEndPoint.Port == 0 ? NetworkConstants.SeedNodePort : ipEndPoint.Port, connectionTimeoutTokenSource.Token).ConfigureAwait(false);
                            
                            var tokenNetwork = IPEndPoint.Parse(_pool.Url);
                            tokenNetwork.Port = NetworkConstants.SeedNodeTokenPort;
                                
                            var json = JsonNode.Parse(await _httpClient.GetStreamAsync($"http://{tokenNetwork}/{NetworkConstants.WalletTokenType}|{NetworkConstants.TokenCheckWalletAddressExist}|{_pool.Username}", connectionTimeoutTokenSource.Token).ConfigureAwait(false));

                            if (json?["result"]?.GetValue<string>().Equals(NetworkConstants.SendTokenCheckWalletAddressInvalid, StringComparison.Ordinal) ?? true)
                            {
                                _getBlockHeaderTcpClient.Dispose();
                        ...
                        else
                        {
                            ...
                            await _getBlockHeaderTcpClient.ConnectAsync(host, hostAndPortMatch.Groups.ContainsKey("port") ? int.Parse(hostAndPortMatch.Groups["port"].Value) : NetworkConstants.SeedNodePort, connectionTimeoutTokenSource.Token);
                            
                            var json = JsonNode.Parse(await _httpClient.GetStreamAsync($"{hostAndPortMatch.Groups["hostname"].Value}:{NetworkConstants.SeedNodeTokenPort}/{NetworkConstants.WalletTokenType}|{NetworkConstants.TokenCheckWalletAddressExist}|{_pool.Username}", connectionTimeoutTokenSource.Token));
            ...
            if (IsNetworkActive && !IsNetworkConnected)
            {
    ...
    {
        using var certificateArrayPoolOwner = GenerateCertificate(out var length);
        ...
        using (var pbkdf1 = new PBKDF1(certificate[..length], salt))
        {
        ...
        SendPacketToNetwork(new PacketData(certificate[..length], false));
    ...
    private async void HandlePacketData()
    {
        try
        {
            do
            {
                ...
                {
                    using var packet = packetHandler;
                    return packet.Execute(_getBlockHeaderTcpClient!.GetStream(), _networkAesKey, _networkAesIv);
                });
    ...
    {
        var currentBlockIndication = _blockIndication;
        ...
        if (currentBlockIndication == _blockIndication)
        {
            GetNewBlockHeader();
            return;
        }
    ...
    private bool UpdateBlockMethod(ReadOnlySpan<byte> packet)
    {
        ...
        BufferUtility.MemoryCopy(_blockXorKey, tempXorKey, _blockXorKeyLength);

        Span<byte> tempAesKey = stackalloc byte[_blockAesKeySize];
        BufferUtility.MemoryCopy(_blockAesKey, tempAesKey, _blockAesKeySize);

        Span<byte> tempAesIv = stackalloc byte[16];
        BufferUtility.MemoryCopy(_blockAesIv, tempAesIv, 16);

        var blockHeader = new BlockHeaderStack
        {
            Height = _blockHeight,
